...
if (_vehicle isEqualto objNull or !(_vehicle isKindOf 'LandVehicle')) exitwith {hint "Could not find a valid vehicle to attach the speedbomb to"};
if !(attachedObjects _vehicle isEqualTo []) exitwith {hint "There is already a bomb on this car"};
if(playerSide isNotEqualTo civilian) exitWith {hint "You cannot rob a gas station while on duty";}; //make sure player is not cop or medic

_bomb = "Bo_GBU12_LGB" createVehicle [0, 0, 300];
_bomb attachTo [_vehicle, [0, 0, -.5]];
hideObject _bomb;
	...
	[_veh, _thisEventHandler] spawn {
		...
		if (speed _v > 88) then {
			hint parsetext "<t size='1.5' align='center'color='#ff1100'>!!!Speedbomb Armed!!!</t>
				<br/><br/>
				<t>A speedbomb was attached to your car if you drop below</t><t color='#ff1100'> 50KM/H </t> <t>you will blow up</t>";
